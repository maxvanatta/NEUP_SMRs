## V25 Extending time frame to multi-year evaluation
    ...
    def BuildModel(self, DAM = False):
        ...
        def Costs(model):
            return (sum(model.pStartupfixedcost[g]*sum(sum(model.vTurnon[g,m,t] for t in model.T) for m in model.M) for g in model.G) 
                    ...
                    + sum(sum(model.pCap[g]*(model.pCAPEX[g]*model.vModuleS[g,m]*(self.pIR/(1-((1+self.pIR)**(-1*self.pLifetime))))+model.pFOPEX[g]*model.vModuleS[g,m]) for m in model.M) for g in model.G)*(len(self.T)/len(self.pLMP))
                    + sum(model.vGenerator[g]*model.pTESCapacity[g]*self.pTESCapex*(self.pIR/(1-((1+self.pIR)**(-1*self.pLifetime))))*(len(self.T)/len(self.pLMP)) for g in model.G))
...
def DAMmain(Temp = 300, NGCost = 4, dem = 100, lmps = 'SPP_SOUTH_2021.csv',sites = 'Base4_MSR.csv', genInd = None, TES = 0, InitMaxMod = 2, naics = 311221, year = 2021, EP = 10, startHr = 0, days = 1):
    ...
    if args.LimitedMod == True:    # V24 10/26/2022
        ...
        print(S)
        if int(math.ceil((peak/cap)*1.2)) >MaxMod:
            pass
        elif int(math.ceil((peak/cap)*1.2)) ==0:
            MaxMod = 1
        else:
            MaxMod = int(math.ceil((peak/cap)*1.2))
        print(MaxMod)
...
if isinstance(args.Sens,str):
    Facilities = pd.read_csv('ERCOT_sensitivity_set.csv')
